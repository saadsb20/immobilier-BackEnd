...
contract MyLandContract
{
    ...
    function addLand(address propertyOwner,string memory _location, uint _cost) public isOwner
    {
    ...
    function transferEther(address payable rec,uint _amount) public  {
        payable(rec).transfer(_amount);
    }
    ...
    function getLand(address _landHolder, uint _index) public returns (string memory, uint, address,uint, uint)
    {
    ...
    function RemoveFromSale(address _landHolder, uint land_id) public returns (string memory){
